...
{
    if (name[0] == 0)
		throw MoxMxf::ArgExc("Image channel name cannot be an empty string.");
...
void	
ChannelList::erase (const char name[])
{
    if (name[0] == 0)
		throw MoxMxf::ArgExc("Image channel name cannot be an empty string.");

	ChannelMap::iterator itr = _map.find(name);
...
void	
ChannelList::erase (const string &name)
{
    erase (name.c_str());
}
...
ChannelList::operator [] (const char name[])
{
    ...
    if (i == _map.end())
		throw MoxMxf::ArgExc("Cannot find image channel");
